% Used for calculating the multi wheel load profile at k_s=2.6MPa
...
S=0.67;                              %distance between two sleeper beams(m) (1998_Auersch L_Vehicle track interaction and Soil dynamics)
...
G_B=112.5e6;                         %shear modulus of ballast(N/m^2)
G_SB=3*28.752e5;                    %shear modulus of sub-ballast(N/m^2)
H_B=1.4;                          %height of ballast(m)
% H_B=0;                          %height of ballast(m)
H_SB=0;                        %height of sub-ballast(m)

K=2.6e6; %<THE VALUE>                          %subgrade modulus(N/m^2)/subgrade modulus(first parameter) of soil per unit beam length
% K=2.7e6;
